            ...
            if (hasTextExcludingChildren(this)) {
                if (!hasAdditionalHidingTechniques(this)) {
                    ...
                    if(!disabled) //Run the contrast test
                     ...
                     if(contrastData && contrastData.bgImage === "none" && !contrastData.opacity) 
                    {
                         // Vérifier si le contraste est insuffisant et enregistrer une erreur
                         if (contrastData.ratio < contrastData.minReq) {
                         Esd1AddError($(this), ESD1_MESSAGES.Rules.ESD1_ERROR_31, contrastData.ratio, contrastData.fgColor, contrastData.bgColor);
